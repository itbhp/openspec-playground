# OpenSpec Workshop: Spec-Driven Persistence Migrations (FastAPI)

**Duration:** Full day · **Stack:** Python · FastAPI · SQLAlchemy · Uvicorn · Docker Compose
...
- `app/model/employee.py` — the plain dataclass model, fully written
- `app/controller/employee_controller.py` — all five operations, in-memory persistence
- `app/service/employee_service.py` — thin pass-through to `EmployeeRepository`
- `app/repository/in_memory_employee_repository.py` — dict + counter
- No tests exist
...
2. **Propose** — describe what you want to change
   ...
   > "Migrate the Employee persistence layer from in-memory dict to MySQL using SQLAlchemy, Docker Compose for integration testing, contract must be maintained."
...
> "Migrate the Employee persistence layer from MySQL/SQLAlchemy to DynamoDB using boto3. Use LocalStack (Docker Compose service) for local development and testing. Keep all controller and service code unchanged."

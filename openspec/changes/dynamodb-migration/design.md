...
**Single DynamoDB table, `employees`, partition key `id` (Number). Counter lives in the same table as a special item, not a separate table.**
Keeps infrastructure to "one table" for a workshop-sized app. Alternative considered: a separate `employee_counters` table — rejected as unnecessary operational overhead for one counter.

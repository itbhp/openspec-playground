# OpenSpec Workshop: Spec-Driven Persistence Migrations (ASP.NET Core)

**Duration:** Full day · **Stack:** .NET 8 · ASP.NET Core · Entity Framework Core · Docker Compose
...
# Verify the app is running (first dotnet watch compile takes ~20-30s)
...
- `Model/Employee.cs` — the plain POCO, fully written
- `Controllers/EmployeesController.cs` — all five operations, in-memory persistence
- `Services/EmployeeService.cs` — thin pass-through to `IEmployeeRepository`
- `Repositories/InMemoryEmployeeRepository.cs` — Dictionary + counter (Singleton)
- No tests exist
...
2. **Propose** — describe what you want to change
   ...
   > "Migrate the Employee persistence layer from in-memory Dictionary to MySQL using Entity Framework Core, Docker Compose for integration testing, contract must be maintained."
...
> "Migrate the Employee persistence layer from MySQL/EF Core to DynamoDB using the AWS SDK for .NET. Use LocalStack (Docker Compose service) for local development and testing. Keep all controller and service code unchanged."
